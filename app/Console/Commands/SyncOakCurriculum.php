...
class SyncOakCurriculum extends Command
{
    ...
    public function handle(): int
    {
        ...
        // Step 3: Sync each unit and its lessons
        $topicCount  = 0;
        $lessonCount = 0;

        foreach ($unitsData as $yearGroup) {
            $yearTitle = $yearGroup['yearTitle'] ?? 'Unknown Year';
            $yearSlug  = $yearGroup['yearSlug']  ?? '';

            $this->line("  Year: {$yearTitle}");
            ...
            foreach ($yearGroup['units'] ?? [] as $unit) {
                ...
                if (!$lessonsResponse->successful()) {
                    $this->warn("    Could not fetch lessons for unit: {$unitSlug}");
                    continue;
                }

                $lessonsData = $lessonsResponse->json();
                $lessonOrder = 1;
